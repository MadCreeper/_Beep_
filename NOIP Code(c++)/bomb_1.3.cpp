...
void put_density()
{
    ...
    for(int i=0;i<100;i++)
    {
        cout<<internal<<setw(8)<<setprecision(5)<<entropy[i];
...
void update_density() //更新密度
{
	...
	for(int i=0;i<100;i++)
    {
        double d[3]={ (double)den[0][i], (double)den[1][i], (double)den[2][i] };
        double sum=d[0]+d[1]+d[2];
        double p[3]={d[0]/sum,d[1]/sum,d[2]/sum};


        if(sum==0.0) entropy[i]=-1.0;
        else
            entropy[i]=(-p[0]*log(p[0])/log(2)) + (-p[1]*log(p[1])/log(2)) + (-p[1]*log(p[1])/log(2));
    }
}
